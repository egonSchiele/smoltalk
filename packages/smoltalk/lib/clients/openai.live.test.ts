 ...
 *
 * These tests make actual paid API calls. They run on CI only when
 * `pnpm test:live` is invoked with OPENAI_API_KEY set. Excluded from the
...
describe.runIf(haveKey).concurrent("OpenAI (Chat Completions) - real API", () => {
  it("hello world", { timeout: TIMEOUT }, async () => {
    ...
    if (r.success) {
      expect(r.value.output).toBeTruthy();
  ...
  it("tool calling", { timeout: TIMEOUT }, async () => {
    ...
    if (r.success) {
      ...
      // Args may be parsed JSON or string — accept either shape.
