...
- A typed reply (`responseFormat`) from a thinking model now comes back as JSON. node-llama-cpp 3.20 made the Qwen chat wrapper open a `<think>` block at the start of every reply, and a grammar is applied from the first token, so the JSON was written inside the block, filed as thinking, and `output` came back empty. The grammar now lets the model think up to the token that closes the block and holds only what follows to the schema, the way llama.cpp's own server does (`lib/thinkingGrammar.ts`). A model whose wrapper has no thought segment gets the plain schema grammar as before. Requires node-llama-cpp 3.21.1 or later, for token matching in grammars.
