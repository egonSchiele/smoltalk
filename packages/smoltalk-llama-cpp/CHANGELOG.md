...
- What the plugin knows about each model family is one profile in `lib/familyProfiles.ts`, keyed by GGUF architecture: the thinking settings for its wrapper, its reply layout, tool markers that replace the wrapper's, and whether a draft is safe when it samples. Qwen3, Qwen3.5, Gemma 4, and gpt-oss have profiles; any other family gets the default, which behaves as before.
