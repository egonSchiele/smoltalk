...
- New `resolveModel(uriOrPath, cacheDir)` export — resolves/downloads a model reference to a local `.gguf` path via node-llama-cpp's `resolveModelFile`; existing file paths are returned unchanged.
