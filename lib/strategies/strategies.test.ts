...
import { SmolPromptConfig, PromptResult } from "../types.js";
...
describe("JSON serialization", () => {
  ...
  describe("round-trip", () => {
    // Helper: serialize → deserialize → re-serialize and check equality (twice)
    function roundTrip(strategy: { toJSON(): any }) {
      const json1 = strategy.toJSON();
      const restored = strategyIndex.fromJSON(json1);
      const json2 = restored.toJSON();
      expect(json2).toEqual(json1);
      const restored2 = strategyIndex.fromJSON(json2);
    ...
    it("FallbackStrategy with all fallback reasons survives round-trip", () => {
      const original = strategyIndex.fallback("gpt-4o", {
        error: [{ type: "id", params: { model: "gpt-4o-mini" } }],
        timeout: [{ type: "id", params: { model: "gpt-4o-mini" } }],
    ...
    it("nested RaceStrategy inside FallbackStrategy survives round-trip", () => {
      const original = strategyIndex.fallback(
        strategyIndex.race("gpt-4o", "claude-sonnet-4-6"),
        { error: [{ type: "id", params: { model: "gemini-2.0-flash" } }] },
      );
      roundTrip(original);
    });

    it("nested FallbackStrategy inside RaceStrategy survives round-trip", () => {
      const original = strategyIndex.race(
    ...
    it("deeply nested strategies survive round-trip", () => {
      const original = strategyIndex.fallback(
        strategyIndex.race(
          strategyIndex.fallback("gpt-4o", {
            error: [{ type: "id", params: { model: "gpt-4o-mini" } }],
  ...
  describe("fallback() shorthands", () => {
    it("fallback() with string shorthand sets error fallback", () => {
      const strategy = strategyIndex.fallback("gpt-4o", "claude-sonnet-4-6");
      expect(strategy).toBeInstanceOf(FallbackStrategy);
    ...
    it("fallback() with array shorthand sets error fallback list", () => {
      const strategy = strategyIndex.fallback("gpt-4o", ["claude-sonnet-4-6", "gemini-2.5-flash"]);
      expect(strategy).toBeInstanceOf(FallbackStrategy);
      const fs = strategy as FallbackStrategy;
